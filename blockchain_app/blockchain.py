...
from warnings import warn #Used to Warn if Validation fails
...
class Block:
  ...
  def __init__(self, index, timestamp, street_name, date, consideration, town_code, guarantor_lname, \
               guarantor_fname, guarantee_lname, guarantee_fname, signed_doc, previous_hash):
...
Signed Doc: %s;
Previous Hash: %s;
""" % (self.index,str(self.timestamp),str(self.street_name).upper(),str(self.date),str(self.consideration),str(self.town_code).upper(),str(self.guarantor_fname).upper(),str(self.guarantor_lname).upper(),str(self.guarantee_fname).upper(), str(self.guarantee_lname).upper(),str(self.signed_doc), str(self.previous_hash))
...
def next_block(last_block):
  ...
  #Added the new attributes
    return Block(this_index, this_timestamp, this_str_name, this_date, this_consideration, this_town_code,
               this_guarantor_lname,this_guarantor_fname, this_guarantee_lname, this_guarantee_fname, this_signed_doc, this_hash)
...
def validate_blockchain(in_blockchain, original_genesis_hash):
    if in_blockchain[0].hash != original_genesis_hash:
        ...
        for current_position in range(1, len(in_blockchain)):
            ...
            if in_blockchain[previous_position].hash == in_blockchain[current_position].previous_hash:
                i = 1
            else:
...
def Query_BC(blockchain, subs):
    ...
    for i in Index:
        ...
        Gor_name.append("%s %s"%(str(blockchain[int_i].guarantor_fname),str(blockchain[int_i].guarantor_lname)))
        Gee_name.append("%s %s"%(str(blockchain[int_i].guarantee_fname),str(blockchain[int_i].guarantee_lname)))
...
def user_block(blockchain, street_name, consideration, town_code, guarantor_lname, guarantor_fname, guarantee_lname, guarantee_fname, file):
    ...
    #Create Block
    new = Block(this_index, this_timestamp, this_str_name, this_date, this_consideration, this_town_code,
               this_guarantor_lname,this_guarantor_fname, this_guarantee_lname, this_guarantee_fname, this_signed_doc, this_hash)
    ...
    New_row = [this_str_name,'office',this_timestamp,'doc_num','volm','page',this_consideration,this_town_code,this_guarantor_lname\
               ,this_guarantor_fname,this_guarantee_lname,this_guarantee_fname, this_signed_doc]
...
def add_Block():
    ...
    if val == "Blockchain Valid":    
        new = user_block(blockchain,address,consideration,city,guarantor_lname,guarantor_fname,guarantee_lname,guarantee_fname,fake_file)
